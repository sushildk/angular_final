  ...
  if(data){
    const roomsId = data.split(', ');
      roomsId.forEach(element => {
        ...
        this.roomService.getById(element).subscribe(
          (data:any)=>{
...
removeCartRoom(roomId:string){
  ...
  const x = localStorage.getItem('cart')!;   /* Getting cart room from local storage */
 const roomsId = x.split(', ');   /* Local storage ma bhako data string ma hunxa teslai split garera arraymarakheko */
//  console.log('rooms id in remove after split:',roomsId);
 roomsId.forEach(element => {   
   if(element == roomId){  /* checking each cart room id with room id that need to be delete */
     this.removeRoomWithoutRefresh(element)   /* calling function to remove room on UI without refreshing page  */
    // roomsId.splice(i,1)
    // console.log('splice element:',element)
  }else{
    if(saveCart){   /* check if saveCart is empty or not */
      saveCart =saveCart+', '+ element   /* if not empty save one room id on save cart separated by , and spac */
    }else{
      saveCart = element
    }
